...
async function seedEngineeringBooks() {
    ...
    let addedCount = 0;
    // Specific engineering sub-fields to get fresh results
    ...
    console.log("Starting to fetch more engineering books...");

    for (let q of queries) {
        if (addedCount >= 50) break; // 56 + 50 = ~106 books total
        let startIndex = 0;
        while (startIndex < 40 && addedCount < 50) {
            try {
                ...
                for (let item of data.items) {
                    ...
                    addedCount++;
                    console.log(`[${addedCount}/50] Added: ${info.title} (ISBN: ${isbn})`);
                }
                startIndex += 40;
            } catch (e) {
    ...
    console.log(`\n✅ Secondary database seeding complete! Successfully added ${addedCount} more engineering books.`);
    process.exit(0);
}
